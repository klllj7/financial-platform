...
function AiChatPage() {
  ...
  /* 세션(또는 채팅 이력 항목)이 마지막으로 쓴 모델을 드롭다운 값 형태로 변환한다.
     대화 중간에 모델을 바꿔도 되므로, 잠그는 용도가 아니라 이전 대화를 다시
     열었을 때 드롭다운 초기값을 맞춰주는 용도로만 쓴다. */
  ...
  */
  ...
  /* 이전 채팅을 선택하면 DB에 저장된 전체 메시지를 불러오고, 그 대화가 마지막으로 쓴 모델로 드롭다운 초기값을 맞춘다. */
        ...
        <section className="ai-chat-conversation-panel">
          ...
          <div className="ai-chat-tool-selector">
            ...
            {approvedTools.length > 0 ? (
              ...
              <p>
                승인된 AI 모델이 없습니다. 먼저 AI 모델 사용을 신청해 주세요.
              </p>
            )}
          </div>
