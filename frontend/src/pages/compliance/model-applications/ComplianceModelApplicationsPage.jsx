  ...
  const filteredApplications = useMemo(() => {
    ...
    return applications.filter((application) => {
      const text = [application.requestName, application.applicantName, application.department, application.purpose]
        .join(" ")
        .toLowerCase();
      return (statusFilter === "전체" || application.status === statusFilter)
  ...
  return (
    <div className="compliance-applications-page">
      <header className="compliance-applications-heading">
        <div>
          <h2>AI Tool 신청 현황</h2>
          <p>임직원이 신청한 AI Tool을 검토하고 승인 상태를 관리합니다.</p>
        </div>
      ...
      <section className="applications-panel">
        <header className="applications-panel-header">
          ...
          <label className="applications-search"><Search size={16} /><input value={keyword} onChange={(event) => setKeyword(event.target.value)} placeholder="신청자, 부서, AI Tool명 검색" /></label>
        </header>
        ...
        <div className="applications-table-wrap">
          <table className="applications-table">
            <thead><tr><th>신청일</th><th>AI Tool</th><th>신청자 / 부서</th><th>사용 목적</th><th>검토 상태</th><th /></tr></thead>
      ...
      {selectedApplication && (
        <div className="application-modal-backdrop" role="presentation" onMouseDown={handleModalClose}>
          <section className="application-modal" role="dialog" aria-modal="true" onMouseDown={(event) => event.stopPropagation()}>
            <header><div><h3>{selectedApplication.requestName}</h3></div><button type="button" onClick={handleModalClose}>닫기</button></header>
            <dl>
